...
Unicode的出现是为了统一地区性文字编码方案，为解决unicode如何在网络上传输的问题，于是面向传输的众多 UTF标准出现了，顾名思义，UTF-8是Unicode的实现方式之一。
...
[80.[2021-10-12] FCP 是什么？](https://github.com/HJY-xh/plantTrees/issues/461)

<details>
<summary>展开查看</summary>
<pre>

FCP(First Contentful Paint)为首次内容绘制，它是测量感知加载速度的一个以用户为中心的重要指标，因为该指标会在用户首次在屏幕上看到任何内容时，在页面加载实践轴中标记出相应的点，迅捷的 FCP 有助于让用户确信某些事情正在进行。

</pre>
</details>

[81.[2021-10-12] LCP 是什么?](https://github.com/HJY-xh/plantTrees/issues/462)

<details>
<summary>展开查看</summary>
<pre>

LCP(Largest Contentful Paint)为最大内容绘制，它是测量感知加载速度的一个以用户为中心的重要指标，因为该指标会在页面的主要内容基本加载完成时，在页面加载时间轴中标记出相应的点，迅捷的 LCP 有助于让用户确信页面是有效的。

</pre>
</details>

[82.[2021-11-27] Mobx 的原则是什么？](https://github.com/HJY-xh/plantTrees/issues/487)
...
![](https://i.loli.net/2021/11/27/olg84iawqC3YJh1.png)

-   所有的 derivations 将在 state 改变时自动且原子化地更新。因此不可能观察中间值。
-   所有的 dervations 默认将会同步更新，这意味着 action 可以在 state 改变 之后安全的直接获得 computed 值。
...
[83.[2021-11-29] UTF-8 的编码规则是什么？](https://github.com/HJY-xh/plantTrees/issues/489)

<details>
<summary>展开查看</summary>
<pre>

UTF 最大的一个特点就是它是一种变长的编码方式。它可以使用 1-4 个字节表示一个符号，根据不同的符号而变化字节长度。

它的编码规则很简单，只有两条：

-   对于单字节的符号，字节的第一位设为 0，后面七位为这个符号的 Unicode 码。因此对于英语字母，UTF-8 编码和 ASCII 码是相同的
...
| U+10000~U+10FFFF             | 011110??? 10?????? 10?????? 10?????? |

</pre>
</details>

[84.[2021-12-15] 归并排序的核心思想是什么？](https://github.com/HJY-xh/plantTrees/issues/498)

<details>
<summary>展开查看</summary>
<pre>

归并的的核心思想是分治。它把数组从中间划分成两个数组，一直递归把子数组划分成更小的数组，知道数组中元素个数为 1 时进行排序。按大小顺序合并两个数组，接着按照递归的顺序返回，不断合并排好序的数组。

</pre>
</details>

[85.[2022-1-6] 如何理解 socket？](https://github.com/HJY-xh/plantTrees/issues/504)

<details>
<summary>展开查看</summary>
<pre>

它是 TCP/IP 协议的编程模型，通过一系列 API 来访问操作系统的网络协议栈，从而建立 TCP 连接，发送或接收数据。

</pre>
</details>

[86.[2022-1-12] 对 web 标准、可⽤性、可访问性的理解](https://github.com/HJY-xh/plantTrees/issues/514)
...
-   可维护性：一般包含两个层次，意识当系统出现问题时，快速定位并解决问题的成本，成本低则可维护性好。二是代码是否容易被人理解，是否容易修改和增强功能

</pre>
</details>

[87.[2022-2-14] Sass 中混合指令@mixin 和 函数指令 @function 的区别?](https://github.com/HJY-xh/plantTrees/issues/525)

<details>
<summary>展开查看</summary>
<pre>

定义方式不同， @function 需要调用@return 输出结果，而 @mixin 不需要:

```css
@mixin base-font ($value: 20px) {
  font-size: $value;
  color: #ff0000;
}
...
@mixin 是需要 @include 来引用，而且不需要小括号，而 @function 是使用需要小括号来执行的:
...
[88.[2022-2-26] Web 页面性能指标中的 FP 是什么意思？](https://github.com/HJY-xh/plantTrees/issues/529)
...
[89.[2022-3-4] 字节码是什么？](https://github.com/HJY-xh/plantTrees/issues/536)
...
[90.[2022-4-3] 为什么需要响应式？](https://github.com/HJY-xh/plantTrees/issues/551)

<details>
<summary>展开查看</summary>
<pre>

随着页面应用的不断复杂，需要关注和管理的状态越来越多，之前靠被动，分散的管理状态不容易掌控，且容易出错。

为了解决问题，出现了许多方案，其中 Vue 的解决方案就是响应式。

</pre>
</details>

[91.[2022-4-3] 怎么实现响应式？](https://github.com/HJY-xh/plantTrees/issues/552)

<details>
<summary>展开查看</summary>
<pre>

响应式主要解决的问题是：如何让“语言层面上分散的独立数据”在“业务层面”上产生“联系或互为因果”的关系。

但是这种需求在语言上不是“普遍需求”，属于“特殊需求”，这里通过了“代理模式”这种设计模式，在语言层面上满足需求，在 ES5 的时代，是`defineProperty`，ES6 则是`Proxy`。
